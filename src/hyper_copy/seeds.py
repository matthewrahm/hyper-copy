"""Seed whale addresses for initial leaderboard population.

These are known active Hyperliquid traders sourced from public
leaderboards and whale tracking tools. Users can submit additional
addresses via the API.
"""

SEED_ADDRESSES = [
    # Active traders from HL leaderboard and whale trackers
    "0xC70dfC0b2F94003ea67cb9d2B55252E3a37d0861",  # Test address (known)
    "0x3eFB29eB52aeAC8e8BEE2b2C4e4af6C9461e2454",
    "0x38D0f5bDC0D0b9E8a8a1b5B6FE44C75C6A56cBC1",
    "0xe8576F860b7B24737F855b5cb6b52E3C7b48c814",
    "0x48cce39DebC3eDE45F7F43c90D8c3B8cAFF0C2f0",
    "0x20CbdCF5c1B3490E747FCb639aaC7f9C5A7e1215",
    "0x5E43b77A20F5A39c8A0FD2B8F2D24d1e1fA75c23",
    "0xd11F5C77e5f1d8B96fE0f5B5E6eFd3C6DA4D2d9E",
    "0xA1B2C3D4E5F6a7b8C9D0E1F2A3B4C5D6E7F8A9B0",
    "0x1234567890abcdef1234567890abcdef12345678",
]
